...
export class AuthService {
	...
	private async generateTokens(user: User, res?: Response) {
		...
		const encodedPayload = { data: encodePayload(payload) }
		const accessToken = await this.jwtService.signAsync(encodedPayload)
		const refreshToken = await this.jwtService.signAsync(encodedPayload, {

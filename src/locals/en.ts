export default {
	profile: {
		update_error: 'Update error'
	},
	messages: {
		user_not_activated: 'User not activated',
		user_not_verified: 'User not verified',
		user_not_found: 'User not found',
		role_not_found: 'Role not found',
		user_exists: 'The user is already in the database',
		user_banned: 'User banned',
		password_not_corrected: 'Incorrect username or password',
		create_user_error: 'Can not create user',
		create_order_error: 'Can not create order',
		activate_user_error: 'Unable to activate user',
		phone_change_error: 'Failed to change phone',
		password_change_error: 'Failed to change password'
	},
	firm: {
		firm_not_found: 'Firm not found'
	},
	mail: {
		activation_on: 'Account activation on ',
		activation_ref: 'To activate follow the link '
	},
	file: {
		write_error: 'Error writing file',
		delete_error: 'Error when deleting a file'
	}
}
